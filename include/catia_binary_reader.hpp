...
public:
    ...
    [[nodiscard]] float f32_le(std::size_t offset) const {
        return std::bit_cast<float>(u32_le(offset));
    }

    [[nodiscard]] double f64_le(std::size_t offset) const {
        return std::bit_cast<double>(u64_le(offset));
    }
